export default async function handler(req,res){
 ...
 try{
  ...
  if(req.method==='GET'){
   ...
   const upstream=await fetch(`${agentUrl}${qs}`,{method:'GET',headers,signal:AbortSignal.timeout(30000)});
  ...
  const upstream=await fetch(agentUrl,{method:'POST',headers,body:JSON.stringify(body),signal:AbortSignal.timeout(30000)});
