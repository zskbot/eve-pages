...
export default async function handler(req,res){
 ...
 try{const commits=await gh(`/commits?sha=${encodeURIComponent(branch)}&per_page=8`);return res.status(200).json({ok:true,configured:true,repo,branch,commits:(commits||[]).map(c=>({sha:c.sha.slice(0,7),message:c.commit?.message?.split('\n')[0]||'',author:c.commit?.author?.name||c.author?.login||'unknown',date:c.commit?.author?.date||''}))})}catch(e){return res.status(502).json({ok:false,error:e.message})}
}