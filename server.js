...
// IMPORTANT: this is the workspace's local agent bridge. The server itself owns
// /api/agent/chat and forwards to the configured Velclaw backend only when set.
...
const json = (res, status, data) => { res.writeHead(status, { "content-type":"application/json; charset=utf-8", "cache-control":"no-store", "access-control-allow-origin":"*", "access-control-allow-methods":"GET,POST,OPTIONS", "access-control-allow-headers":"content-type,authorization" }); res.end(JSON.stringify(data)); };
...
const listTree = async (dir, rel="") => { const { readdir }=await import("node:fs/promises"); const entries=await readdir(dir,{withFileTypes:true}); const out=[]; for(const e of entries){if(e.name === ".git" || e.name === "node_modules")continue;const rp=rel?`${rel}/${e.name}`:e.name;if(e.isDirectory())out.push({name:e.name,path:rp,type:"dir",children:await listTree(path.join(dir,e.name),rp)});else out.push({name:e.name,path:rp,type:"file"});}return out; };

async function agentChat(body){
  if(!AGENT_URL)return {ok:false,configured:false,error:"Velclaw Agent backend is not configured. Set VELCLAW_AGENT_URL."};
  ...
  const text=await r.text();let data;try{data=JSON.parse(text);}catch{data={response:text};}
...
async function api(req,res,url){
  ...
  if(url.pathname === "/api/workspace/file" && req.method === "POST"){
    try{const b=JSON.parse(await readBody(req));if(!b.path||typeof b.content!=="string")return json(res,400,{error:"path and string content are required"});const full=safePath(b.path);const {mkdir}=await import("node:fs/promises");await mkdir(path.dirname(full),{recursive:true});await writeFile(full,b.content,"utf8");return json(res,200,{ok:true,path:b.path,mtime:(await stat(full)).mtime.toISOString()});}catch(e){return json(res,400,{error:e.message});}
  }
  if(url.pathname === "/api/workspace/tree" && req.method === "GET")return json(res,200,{ok:true,root:ROOT,tree:await listTree(ROOT)});
  if(url.pathname === "/api/workspace/status" && req.method === "GET")return json(res,200,{ok:true,workspace:ROOT,agent:{configured:Boolean(AGENT_URL),url:AGENT_URL?new URL(AGENT_URL).origin:null},server:{pid:process.pid,node:process.version,uptime:process.uptime()},time:new Date().toISOString()});
...
const server=http.createServer(async(req,res)=>{
  try{const url=new URL(req.url,`http://${req.headers.host||"localhost"}`);if(url.pathname.startsWith("/api/")){const handled=await api(req,res,url);if(handled!==false)return;return json(res,404,{error:"api route not found"});}let p=decodeURIComponent(url.pathname);if(p==="/")p="/workspace.html";const full=safePath(p.replace(/^\//,""));const s=await stat(full);if(!s.isFile())return json(res,404,{error:"not found"});res.writeHead(200,{"content-type":MIME[path.extname(full).toLowerCase()]||"application/octet-stream","cache-control":"no-cache"});res.end(await readFile(full));}catch(e){if(!res.headersSent)json(res,404,{error:e.message});else res.end();}
});
